# BULWARK - Hans Richter

**[← Back to Tank Class](./README.md)** | **[All Operators](../Operators.md)**

---

## Operator Profile

> *"Behind me. Nothing gets through."*

### Basic Information

| Attribute       | Value                  |
| :-------------- | :--------------------- |
| **Real Name**   | Hans Friedrich Richter |
| **Codename**    | BULWARK                |
| **Class**       | Tank                   |
| **Nationality** | German                 |
| **Age**         | 41                     |
| **Height**      | 195 cm (6'5")          |
| **Weight**      | 110 kg (243 lbs)       |

### Background

Hans Richter was the immovable object of GSG 9, Germany's elite counter-terrorism unit. His ability to hold positions under overwhelming fire became legendary after a 2019 embassy siege where he protected 30 hostages for 8 hours with only a riot shield and his determination.

After a political scandal forced budget cuts that disbanded his unit, Hans couldn't return to civilian life. The Exclusion Zone offers what he needs: a clear mission, enemies to stop, and people to protect.

### Personality Traits

- **Stoic** - Rarely shows emotion
- **Protective** - Lives for defending others
- **Stubborn** - Will not retreat
- **Honorable** - Respects worthy opponents

---

## Combat Statistics

### Base Stats

| Stat             | Value   | Class Modifier | Final        |
| :--------------- | :------ | :------------- | :----------- |
| **Health**       | 100 HP  | -              | 100 HP       |
| **Armor**        | 75      | +25% cap       | 75 (max 125) |
| **Sprint Speed** | 5.5 m/s | -15%           | 4.675 m/s    |
| **Crouch Speed** | 2.0 m/s | -              | 2.0 m/s      |

---

## Abilities

### Active Ability: Riot Shield Deploy

> *"Deploy an indestructible riot shield. Cannot fire while active."*

| Property            | Value            |
| :------------------ | :--------------- |
| **Cooldown**        | 80 seconds       |
| **Duration**        | 15 seconds       |
| **Shield Coverage** | 180° frontal arc |

#### Effects

| Effect           | Value          | Notes             |
| :--------------- | :------------- | :---------------- |
| Damage Block     | 100% frontal   | Cannot be broken  |
| Movement Penalty | -40%           | Very slow         |
| Weapon Use       | Secondary only | While shield up   |
| Melee            | Yes            | Shield bash stuns |

#### Shield Bash

| Property      | Value                     |
| :------------ | :------------------------ |
| Damage        | 30                        |
| Stun Duration | 1.5 seconds               |
| Range         | 2 meters                  |
| Cooldown      | 5 seconds (during shield) |

#### Upgrade Slots

**Slot 1 (Level 5):**
| Option             | Effect                            |
| :----------------- | :-------------------------------- |
| **Extended Guard** | Duration +5 seconds (20s total)   |
| **Quick Deploy**   | Cooldown -20 seconds (60s total)  |
| **Mobile Wall**    | Movement penalty -15% (only -25%) |

**Slot 2 (Level 20):**
| Option                 | Effect                               |
| :--------------------- | :----------------------------------- |
| **Shield Bash+**       | Bash damage +20, stun +0.5 sec       |
| **Reflective Surface** | 10% damage reflected back to shooter |
| **Steadfast**          | Immune to stagger/knockback          |

**Slot 3 (Level 35):**
| Option                   | Effect                                |
| :----------------------- | :------------------------------------ |
| **Forward Unto Dawn**    | Gain 50 temp HP when deploying        |
| **Team Aura**            | Allies behind shield gain +10% damage |
| **Explosive Resistance** | Block 50% grenade damage              |

---

### Passive Ability: Living Wall

> *"The bigger they are, the harder they stand."*

| Condition      | Effect                                             |
| :------------- | :------------------------------------------------- |
| Standing still | +15% damage resistance                             |
| In cover       | +20% damage resistance                             |
| Taking fire    | Generate Aggro (enemies more likely to target you) |

---

## Loadout

### Default Loadout

| Slot          | Item                            | Notes              |
| :------------ | :------------------------------ | :----------------- |
| **Primary**   | SPAS-12 Shotgun                 | CQB power          |
| **Secondary** | .44 Magnum                      | High damage        |
| **Tactical**  | Flashbang ×1, Extra Armor Plate | Entry + durability |
| **Armor**     | Heavy Vest                      | 75 armor           |

---

## Voice Lines

### Combat

| Trigger            | Line                              |
| :----------------- | :-------------------------------- |
| Ability Activation | "Shield up! I'll cover you!"      |
| Shield Bash        | "Back!"                           |
| Taking Heavy Fire  | "I can take it. Keep moving!"     |
| Shield Down        | "Shield dropping. Need a moment." |

### Personality

| Trigger            | Line                            |
| :----------------- | :------------------------------ |
| Match Start        | "I go first. Stay behind me."   |
| Extraction Success | "Mission complete. All secure." |

---

## Lore Connections

| Character | Relationship                          |
| :-------- | :------------------------------------ |
| **VIPER** | Rival philosophies, mutual respect    |
| **DOC**   | Old friends, saved each other's lives |
| **HAVOC** | Professional animosity, too reckless  |

---

**[← Back to Tank Class](./README.md)** | **[All Operators](../Operators.md)**
