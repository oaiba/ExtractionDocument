# Recon Class - Information Specialists

**[← Back to Operators](../Operators.md)** | **[Characters Index](../README.md)**

---

## Class Overview

The **Recon** class operators excel at gathering intelligence and controlling information flow. They enable ambushes, prevent surprises, and dominate through knowledge.

### Class Identity

| Attribute           | Value           | Notes                      |
| :------------------ | :-------------- | :------------------------- |
| **Role**            | Intel & Stealth | Information is power       |
| **Difficulty**      | ★★★★☆           | High - Requires game sense |
| **Team Dependency** | Medium          | Intel helps team, can solo |
| **Skill Ceiling**   | Very High       | Map knowledge crucial      |

### Class Traits (All Recon Operators)

| Trait               | Effect               | Gameplay Impact         |
| :------------------ | :------------------- | :---------------------- |
| **Sneaky Movement** | +15% Crouch Speed    | Faster stealth movement |
| **Silent Steps**    | -30% Footstep Volume | Harder to hear coming   |
| **Fragile Frame**   | -5% Maximum Health   | Glass cannon            |

---

## Operators

| Operator                          | Codename | Specialty      | Unlock                   |
| :-------------------------------- | :------- | :------------- | :----------------------- |
| [**Sarah Kim**](./Phantom.md)     | PHANTOM  | Area Reveal    | Level 8, 5,000 Credits   |
| [**Viktor Volkov**](./Specter.md) | SPECTER  | Trap Detection | Level 12, 7,500 Credits  |
| [**Yuki Tanaka**](./Wraith.md)    | WRAITH   | Visual Denial  | Level 25, 15,000 Credits |

---

## Playstyle Guide

### When to Pick Recon

✅ **Pick Recon when:**
- Solo or small team
- Map has lots of flank routes
- You know the map well
- Enemy team has ambush potential

❌ **Avoid Recon when:**
- Open, simple maps
- Your team needs healing
- You're unfamiliar with routes
- Enemy has heavy area denial

### Information Priority

```
1. Enemy positions (before fight)
2. Extraction zone safety
3. Loot area clearance
4. Flank watch
```

---

## Counter Strategies

### How to Counter Recon Operators

| Operator    | Counter Strategy                        |
| :---------- | :-------------------------------------- |
| **PHANTOM** | Stay moving, don't get scanned in place |
| **SPECTER** | Don't trigger traps, check corners      |
| **WRAITH**  | Push through smoke, don't wait          |

---

## Recon Operator Quick Links

- 📄 [PHANTOM - Sarah Kim](./Phantom.md)
- 📄 [SPECTER - Viktor Volkov](./Specter.md)
- 📄 [WRAITH - Yuki Tanaka](./Wraith.md)

---

**[← Back to Operators](../Operators.md)** | **[Characters Index](../README.md)**
