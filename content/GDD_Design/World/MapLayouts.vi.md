---
title: "Map Layouts - Detailed Map Configurations"
type: docs
---

## Points of Interest (POI)

### POI Tiers

| Tier  | Name      | Characteristics                         | Quantity/Map |
| :---: | :-------- | :-------------------------------------- | :----------: |
| **S** | Landmark  | Major visual landmark, hiển thị rõ map-wide |     1-2      |
| **A** | Major POI | primary area với high-tier loot        |     3-4      |
| **B** | Minor POI | secondary area, moderate loot           |     5-7      |
| **C** | Micro POI | Small point, basic loot                 |     8-12     |

### Industrial Decay POIs

**S-Tier: Reactor Tower**
- Position: Map Center (800m, 800m)
- Height: 45m (tallest), 4 floors + basement
- Loot: Epic Vaults, Legendary Cache
- AI: 15-20 địch + Boss

**A-Tier:**
- Tech Labs Complex (East) - 3 buildings, quest items
- Warehouse District (West) - 6 warehouses, cơ chế parts
- Office Complex (North) - 5 floors, Intel documents

---

## Hotspots - High Activity Zones

### Hotspot 1: Reactor cốt lõi
**Heat:**  | Combat rate: 85% | Deaths/match: 8-12

**Entry Points (7):** Main entrance, Emergency stairs, dịch vụ ladder, Elevator, Roof access, Vent, Basement tunnel

### Hotspot 2: The Bridge
**Heat:**  | Combat rate: 70% | Deaths/match: 4-6

**tactical Overview:** Crossing point between Warehouse và Office. Vulnerable to sniper fire from Office windows.

### Hotspot 3: Tech Labs Lobby
**Heat:**  | Combat rate: 55% | Deaths/match: 3-5

**Tactical Overview:** Single main entrance với a 3-floor atrium providing multiple vertical sightlines.

---

## Choke Points

### Critical Chokes

| ID   | Location         | Width | Type       | Bypass        |
| :--- | :--------------- | :---- | :--------- | :------------ |
| CH-1 | The Bridge       | 3m    | Structural | Canal, Tunnel |
| CH-2 | Reactor Entrance | 4m    | Structural | Vent, Ladder  |
| CH-3 | Lab Lobby        | 5m    | Vertical   | Fire escape   |

### secondary Chokes

| ID   | Location         | Width | ghi chú           |
| :--- | :--------------- | :---- | :-------------- |
| CH-4 | Warehouse W2     | 2m    | Roll-up shutter |
| CH-5 | Office Stairwell | 1.5m  | 5 floor funnel  |
| CH-6 | Water Pipe       | 1m    | Crouch required |

---

## Extraction Points

### EX-1: Helipad (North)
- **Type:** Signal (Flare Gun required)
- **Capacity:** 4 | **Wait:** 90s | **Risk:**  High
- hiển thị rõ map-wide, high PvP target

### EX-2: Gate 03 (East)
- **Type:** Always Open
- **Capacity:** 6 | **Wait:** 15s | **Risk:**  Medium
- Most used extraction

### EX-3: Convoy (South)
- **Type:** Paid (3000 Credits)
- **Capacity:** 8 | **Wait:** 30s | **Risk:**  Medium

### EX-4: Sewer (Central)
- **Type:** Conditional (No Backpack)
- **Capacity:** 2 | **Wait:** 45s | **Risk:**  Low
- Emergency escape, hidden location

### EX-5: Boat (West)
- **Type:** Timer (<3:00 remaining)
- **Capacity:** 3 | **Wait:** 20s | **Risk:**  High
- Late-game emergency option

---

## Traffic flow

**Early Game (0-5m):** Spawn → Rush to nearest POI → Move towards Mid zones
**Mid Game (5-10m):** Contesting hot zones, fighting for Supply drops, Peak PvP encounters
**Late Game (10-15m):** Securing escape routes → Moving to Extraction Points
